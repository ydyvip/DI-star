...
from distar.ctools.torch_utils.network import activation
import torch
import torch.nn as nn
import torch.nn.functional as F
from distar.ctools.torch_utils import fc_block, conv2d_block, deconv2d_block, build_activation, ResBlock, NearestUpsample, BilinearUpsample
from distar.ctools.torch_utils.network.rnn import sequence_mask
from ..module_utils import GatedResBlock, FiLMedResBlock, AttentionPool
...
class SelectedUnitsHead(nn.Module):
    def __init__(self, cfg):
        ...
        if self.whole_cfg.model.entity_reduce_type == 'attention_pool':
            ...
            self.attention_pool = AttentionPool(key_dim=self.cfg.key_dim, head_num=2, output_dim=self.cfg.input_dim, max_num=MAX_SELECTED_UNITS_NUM + 1)
    ...
    def _get_key_mask(self, entity_embedding, entity_num):
        ...
        if self.whole_cfg.model.entity_reduce_type == 'entity_num':
            ...
            key_embeddings = self.embed_fc(key_reduce)
        elif self.whole_cfg.model.entity_reduce_type == 'selected_units_num' or 'attention' in self.whole_cfg.model.entity_reduce_type:
            key_embeddings = key
        else:
    ...
    def _query(
            self, key: Tensor, entity_num: Tensor, autoregressive_embedding: Tensor, logits_mask: Tensor,
            key_embeddings: Tensor, selected_units_num: Optional[Tensor] = None,
        ...
        state = [(torch.zeros(ae.shape[0], 32, device=ae.device), torch.zeros(ae.shape[0], 32, device=ae.device)) for _
                ...
                if self.whole_cfg.model.entity_reduce_type == 'selected_units_num' or 'attention' in self.whole_cfg.model.entity_reduce_type:
                    ...
                    if self.whole_cfg.model.entity_reduce_type == 'selected_units_num':
                        ...
                        selected_units_emebedding[selected_units_num != 0] = selected_units_emebedding[selected_units_num != 0] / \
                                                    new_selected_units_one_hot.sum(dim=1)[selected_units_num != 0]
                        ...
                        ae = autoregressive_embedding + self.attention_pool(key_embeddings, mask=new_selected_units_one_hot)
                    elif self.whole_cfg.model.entity_reduce_type == 'attention_pool_add_num':
                        ae = autoregressive_embedding + self.attention_pool(key_embeddings, num=new_selected_units_one_hot.sum(dim=1).squeeze(dim=1),
                    ...
                    for i in range(seq_len):
                        ...
                        if self.whole_cfg.model.entity_reduce_type == 'selected_units_num' or 'attention' in self.whole_cfg.model.entity_reduce_type:
                            ...
                            if self.whole_cfg.model.entity_reduce_type == 'selected_units_num':
                                ...
                                iou_ae = autoregressive_embedding + self.attention_pool(key_embeddings,
                                                                                    mask=selected_units_one_hot)
                            elif self.whole_cfg.model.entity_reduce_type == 'attention_pool_add_num':
                                iou_ae = autoregressive_embedding + self.attention_pool(key_embeddings,
                                                                                    num=selected_units_one_hot.sum(
                                                                                        dim=1).squeeze(dim=1),
                                                                                    mask=selected_units_one_hot, )
                        else:
            ...
            for i in range(self.max_select_num):
                ...
                if self.whole_cfg.model.entity_reduce_type == 'selected_units_num' or 'attention' in self.whole_cfg.model.entity_reduce_type:
                    ...
                    if self.whole_cfg.model.entity_reduce_type == 'selected_units_num':
                        ...
                        selected_units_emebedding[slected_num != 0] = selected_units_emebedding[slected_num != 0] / slected_num[slected_num != 0].unsqueeze(dim=1)
                        selected_units_emebedding = self.embed_fc2(self.embed_fc1(selected_units_emebedding))
                        ae = autoregressive_embedding + selected_units_emebedding
                    elif self.whole_cfg.model.entity_reduce_type == 'attention_pool':
                        ae = autoregressive_embedding + self.attention_pool(key_embeddings,
                                                                                mask=selected_units_one_hot)
                    elif self.whole_cfg.model.entity_reduce_type == 'attention_pool_add_num':
                        ae = autoregressive_embedding + self.attention_pool(key_embeddings,
                                                                                num=selected_units_one_hot.sum(
...
class TargetUnitHead(nn.Module):
    ...
    def forward(
        ...
        entity_embedding,
        entity_num,
        target_unit: Optional[torch.Tensor] = None
    ):
...
class LocationHead(nn.Module):
    def __init__(self, cfg):
        ...
        self.conv1 = conv2d_block(
            self.cfg.map_skip_dim + self.cfg.reshape_channel, self.cfg.res_dim, 1, 1, 0, activation=build_activation(self.cfg.activation), norm_type=None
        )
        ...
        self.project_embed = fc_block(self.cfg.input_dim, self.whole_cfg.model.spatial_y // 8 * self.whole_cfg.model.spatial_x // 8 * 4, activation=build_activation(self.cfg.activation))
        if self.use_film:
        ...
        for i in range(self.cfg.res_num):
            if self.use_gate:
                self.res.append(GatedResBlock(self.res_dim, self.res_dim, 3, 1, 1, activation=build_activation(self.cfg.activation), norm_type=None))
            else:
        ...
        for i in range(len(self.cfg.upsample_dims)):
            ...
            if self.cfg.upsample_type == 'deconv':
                self.upsample.append(deconv2d_block(dims[i], dims[i + 1], 4, 2, 1, activation=activation, norm_type=None))
            else:
    ...
    def forward(self, embedding, map_skip: List[Tensor], location=None):
        ...
        reshape_embedding = projected_embedding.reshape(projected_embedding.shape[0], self.reshape_channel, self.whole_cfg.model.spatial_y // 8, self.whole_cfg.model.spatial_x // 8)
