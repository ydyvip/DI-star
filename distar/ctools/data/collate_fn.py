...
def default_collate_with_dim(batch,device='cpu',dim=0, k=None):
    ...
    if isinstance(elem, torch.Tensor):
        ...
        if elem_type.__name__ == 'ndarray' or elem_type.__name__ == 'memmap':
            ...
            return default_collate_with_dim([torch.as_tensor(b,device=device) for b in batch],device=device,dim=dim)
        elif elem.shape == ():  # scalars
        ...
        return {key: default_collate_with_dim([d[key] for d in batch if key in d.keys()],device=device,dim=dim, k=key) for key in elem}
    elif isinstance(elem, tuple) and hasattr(elem, '_fields'):  # namedtuple
        return elem_type(*(default_collate_with_dim(samples,device=device,dim=dim) for samples in zip(*batch)))
    elif isinstance(elem, container_abcs.Sequence):
        ...
        transposed = zip(*batch)
        return [default_collate_with_dim(samples,device=device,dim=dim) for samples in transposed]
